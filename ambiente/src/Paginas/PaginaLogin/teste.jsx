...
// Definição da função Http
const Http = (method, data) => {
  return {
    method: method,
    headers: {
...
function Login() {
  ...
  const verificaLogin = async (e) => {
    ...
    const user = {
      email: email,
      senha: password, // Certifique-se de usar 'senha' se o backend espera esse campo
    };
    ...
    try {
      ...
      const response = await fetch("https://vel-tnpo.onrender.com/login/usuario", Http("POST", user));
      ...
      const responseData = await response.text(); // Alterado para response.json()
